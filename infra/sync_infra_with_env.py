...
def extract_resource_info(config):
    ...
    if ai_endpoint:
        match = re.search(r'https://([^.]+)\.services\.ai\.azure\.com', ai_endpoint)
        if match:
    ...
    info['gpt_deployment'] = config.get('AZURE_OPENAI_CHAT_DEPLOYMENT_NAME', 'gpt-5-mini')
    info['embedding_deployment'] = config.get('AZURE_OPENAI_EMBEDDING_DEPLOYMENT_NAME', 'text-embedding-3-small')
...
def main():
    ...
    print(f"   • AI Service: {resource_info.get('ai_service_name', 'Not detected')}")
    print(f"   • OpenAI Service: {resource_info.get('openai_name', 'Not detected')}")
    print(f"   • Search Service: {resource_info.get('search_name', 'Not detected')}")
    print(f"   • GPT Deployment: {resource_info.get('gpt_deployment', 'Not detected')}")
    print(f"   • Embedding Deployment: {resource_info.get('embedding_deployment', 'Not detected')}")
