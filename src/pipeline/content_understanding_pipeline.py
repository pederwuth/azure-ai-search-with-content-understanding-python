...
from ..core.exceptions import PipelineError, ContentUnderstandingError
...
class ContentUnderstandingPipeline:
    ...
    def __init__(self, settings: Optional[Settings] = None):
        ...
        if OPENAI_AVAILABLE:
            ...
            logger.warning("OpenAI client not available - summarization features disabled")
        
        self.document_processor = DocumentProcessor(self.settings, self.credential)
    ...
    def process_pdf_with_understanding(
        ...
        try:
            ...
            logger.info(f"DEBUG: output_dir parameter = {output_dir}")
            ...
            logger.info(f"DEBUG: output_path resolved to = {output_path.absolute()}")
            
            if figures_dir is None:
                figures_dir = str(output_path / "figures")
            
            logger.info(f"DEBUG: figures_dir = {figures_dir}")
            
            # Get analyzer template path
            analyzer_template_path = self._get_analyzer_template_path(analyzer_template)
            
            # Process PDF to enhanced markdown
            logger.info("Processing PDF with Document Intelligence and Content Understanding")
            ...
            # Save enhanced markdown with proper filename
            # Use original filename if available, otherwise use PDF basename
            original_filename = getattr(pdf_file, '_original_name', pdf_file.name)
            ...
            if generate_summary:
                if not self.openai_client:
                    logger.warning("OpenAI client not available - skipping summary generation")
                    results.update({
                    ...
                    try:
                        book_summary = self._generate_book_summary(enhanced_markdown, pdf_file.stem)
                        
                        # Save summary
                        summary_file = output_path / f"{pdf_file.stem}_summary.json"
                        self.file_manager.save_book_summary(book_summary, str(summary_file))
    ...
    def process_multiple_pdfs(
        ...
        logger.info(f"Batch processing completed. {len([r for r in results.values() if r.get('processing_status') == 'success'])} successful, {len([r for r in results.values() if r.get('processing_status') == 'failed'])} failed")
    ...
    def _generate_book_summary(self, markdown_content: str, book_title: str) -> BookSummary:
        ...
        """
        if not self.openai_client:
            raise PipelineError("OpenAI client not available for summary generation")
        
        # Split content into chapters (simple approach - could be enhanced)
        chapters = self._split_content_into_chapters(markdown_content, book_title)
    ...
    def extract_figures_only(
        ...
        try:
            ...
            logger.info(f"Extracted and analyzed {len(figure_contents)} figures")
