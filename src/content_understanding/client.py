...
class ContentUnderstandingClient:
    ...
    def __init__(
        ...
        try:
            ...
            logger.info("Content Understanding client initialized successfully")
    ...
    def analyze_image(self, analyzer_id: str, image_path: str, timeout_seconds: int = 300) -> Dict[str, Any]:
        ...
        """
        try:
            logger.debug(f"Analyzing image: {image_path} with analyzer: {analyzer_id}")
            response = self._client.begin_analyze(analyzer_id, image_path)
            result = self._client.poll_result(response, timeout_seconds=timeout_seconds)
