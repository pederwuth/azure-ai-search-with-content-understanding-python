...
class DocumentProcessor:
    ...
    def process_pdf_to_markdown(
        ...
        try:
            ...
            if result.figures and analyzer_template_path:
                logger.info(f"Processing {len(result.figures)} figures with Content Understanding")
                ...
                md_content = self._insert_figure_contents(md_content, figure_contents, span_offsets)
                
                logger.info("Figure contents inserted into markdown")
            else:
                logger.info("No figures found or no analyzer template provided")
            
            logger.info(f"Document processing completed. Markdown length: {len(md_content):,} characters")
            return md_content
        ...
        try:
            ...
            for figure_idx, figure in enumerate(figures):
                try:
                    ...
                    cropped_img = self._crop_image_from_pdf_page(pdf_path, page_number - 1, bounding_box)
                    ...
                    result = content_client.analyze_image(analyzer_id, str(figure_filepath))
                    
                    # Format result
                    figure_content = self._format_content_understanding_result(result)
                    figure_contents.append(figure_content)
                    
                    logger.debug(f"Figure {figure_idx + 1} processed successfully")
                    
                except Exception as e:
                    logger.warning(f"Failed to process figure {figure_idx + 1}: {e}")
                    figure_contents.append(f"Figure {figure_idx + 1}: Content analysis failed")
    ...
    def _crop_image_from_pdf_page(
        ...
        try:
            ...
            raise DocumentProcessingError(f"Failed to crop image from PDF: {e}") from e
    ...
    def _insert_figure_contents(
        ...
        """
        # Debug logging
        logger.info(f"DEBUG: span_offsets = {span_offsets}")
        logger.info(f"DEBUG: figure_contents count = {len(figure_contents)}")
        logger.info(f"DEBUG: md_content length = {len(md_content)}")
        ...
        for i, offset in enumerate(span_offsets):
            logger.info(f"DEBUG: Processing span {i}: offset={offset}, span_offsets length={len(span_offsets)}")
            
            if i == 0 and offset > 0:
                preamble = md_content[0:offset]
                logger.info(f"DEBUG: Set preamble (length={len(preamble)})")
                # Check if we have more than one span before accessing [i + 1]
                if i < len(span_offsets) - 1:
                    parts.append(md_content[offset:span_offsets[i + 1]])
                    logger.info(f"DEBUG: Added part from {offset} to {span_offsets[i + 1]}")
                else:
                    parts.append(md_content[offset:])
                    logger.info(f"DEBUG: Added final part from {offset} to end")
        ...
        logger.info(f"DEBUG: Successfully processed {len(parts)} parts")
    ...
    def _format_content_understanding_result(self, content_understanding_result: Dict[str, Any]) -> str:
        ...
        def _format_result(key: str, result: Dict[str, Any]) -> str:
            ...
            if result_type in ["string", "integer", "number", "boolean"]:
                ...
                values = [
                    str(result["valueArray"][i][f"value{r['type'].capitalize()}"])
        ...
        try:
            ...
            logger.debug(f"Content Understanding result keys: {list(content_understanding_result.keys())}")
            if 'result' in content_understanding_result:
                logger.debug(f"Result keys: {list(content_understanding_result['result'].keys())}")
                if 'contents' in content_understanding_result['result']:
                    logger.debug(f"Contents length: {len(content_understanding_result['result']['contents'])}")
                    if content_understanding_result['result']['contents']:
                        logger.debug(f"First content keys: {list(content_understanding_result['result']['contents'][0].keys())}")
            ...
            if 'markdown' in first_content:
                ...
                markdown_content = markdown_content.replace('<figure>', '').replace('</figure>', '')
                ...
                for field_name in fields:
                    markdown_result += _format_result(field_name, fields[field_name])
                return markdown_result
            else:
                return f"Unsupported content structure: {list(first_content.keys())}"
            
        except (KeyError, IndexError) as e:
            logger.warning(f"Failed to format Content Understanding result: {e}")
            logger.warning(f"Content Understanding result structure: {content_understanding_result}")
