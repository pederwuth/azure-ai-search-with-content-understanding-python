...
class SummarizationServiceClient:
    ...
    def __init__(self, base_url: str = "http://localhost:8001", timeout: int = 300):
        ...
        logger.info(
            f"Initialized SummarizationServiceClient with base_url: {base_url}")
    ...
    def summarize_markdown(self, markdown_content: str, book_title: str) -> BookSummary:
        ...
        try:
            logger.info(
                f"Sending summarization request for book: {book_title}")
            ...
            for cs_data in result.get('chapter_summaries', []):
                ...
                if 'created_at' in cs_data and isinstance(cs_data['created_at'], str):
                    cs_data['created_at'] = datetime.fromisoformat(
                        cs_data['created_at'].replace('Z', '+00:00'))
            ...
            logger.info(
                f"✅ Successfully received summary for '{book_title}' with {len(book_summary.chapter_summaries)} chapters")
            return book_summary
            ...
            raise ValueError(
                f"Invalid response from summarization service: {e}")
    ...
    def summarize_file(self, file_path: str, book_title: Optional[str] = None) -> BookSummary:
        ...
        try:
            ...
            logger.info(
                f"✅ Successfully received summary for file '{file_path_obj.name}' with {len(book_summary.chapter_summaries)} chapters")
            return book_summary
