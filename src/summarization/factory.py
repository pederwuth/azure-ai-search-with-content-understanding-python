...
class SummarizationFactory:
    ...
    def _check_service_availability(self) -> bool:
        ...
        try:
            ...
            logger.warning("❌ Summarization service is not available after retries")
    ...
    def _should_use_service(self) -> bool:
        ...
        if self.config.fallback_to_direct:
            logger.info("📍 Falling back to direct imports (service unavailable)")
            return False
        
        raise RuntimeError("Summarization service unavailable and fallback disabled")
