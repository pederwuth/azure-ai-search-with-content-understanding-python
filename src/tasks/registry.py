...
class TaskRegistry:
    ...
    def register_task(self, task_class: Type[BaseTask]) -> None:
        ...
        if metadata.task_id in self._tasks:
            self.logger.warning(
                f"Task {metadata.task_id} is already registered, overwriting")
    ...
    def get_task(self, task_id: str) -> BaseTask:
        ...
        if task_id not in self._tasks:
            raise ValueError(
                f"Task '{task_id}' is not registered. Available tasks: {list(self._tasks.keys())}")
    ...
    def resolve_dependencies(self, target_tasks: List[str]) -> List[str]:
        ...
        queue = deque(
            [task_id for task_id in all_tasks if in_degree[task_id] == 0])
        execution_order = []
        ...
        try:
            ...
            if not issues:
                ...
                for task_id in resolved_order:
                    ...
                    required_inputs = set(
                        t.value for t in metadata.input_types)
                    if required_inputs and not (required_inputs & available_outputs):
                        ...
                        if task_id not in task_ids or available_outputs:
                            issues.append(
                                f"Task '{task_id}' requires inputs {required_inputs} but only {available_outputs} are available")

                    # Add this task's outputs to available outputs
                    available_outputs.update(
                        t.value for t in metadata.output_types)
