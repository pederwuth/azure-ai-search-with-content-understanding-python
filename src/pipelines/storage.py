...
class PipelineStorage:
    ...
    def load_pipeline_result(self, pipeline_id: str) -> Optional[PipelineResult]:
        ...
        try:
            ...
            self.logger.error(f"Failed to load pipeline result {pipeline_id}: {e}")
            return None
    ...
    def list_pipelines(self, 
        ...
        try:
            ...
            if status:
                pipelines = [p for p in pipelines if p.get('status') == status.value]
    ...
    def _deserialize_result(self, data: Dict[str, any]) -> PipelineResult:
        ...
        for task_id, exec_data in data.get("task_executions", {}).items():
            execution = TaskExecution(
                ...
                start_time=datetime.fromisoformat(exec_data["start_time"]) if exec_data["start_time"] else None,
    ...
    def _update_pipeline_index(self, result: PipelineResult) -> None:
        ...
        pipelines = [p for p in pipelines if p.get('pipeline_id') != result.pipeline_id]
    ...
    def _remove_from_index(self, pipeline_id: str) -> None:
        ...
        try:
            ...
            # Remove the pipeline
            pipelines = [p for p in pipelines if p.get('pipeline_id') != pipeline_id]
